...
function CRMDashboard(props) {
  ...
  useEffect(() => {
    if (user && user.company_id) {
      ...
      Promise.all([
        axios.get(
          `http://localhost:8081/opportunities/company`,{
          headers: {
            "Authorization": `Bearer ${token}`
          },
          withCredentials: true
        }),
        axios.get(`http://localhost:8081/contacts/company`, {
          headers: {
            "Authorization": `Bearer ${token}`
          },
          withCredentials: true
  ...
  const handleBlur = (id, field) => {
    ...
    if (field === "status") {
      if (oppToUpdate.status === "won") {
        ...
        // Set chance_of_winning to 100% for "won"
        oppToUpdate.chance_of_winning = 100;
      } else if (oppToUpdate.status === "lost") {
        // Set chance_of_winning to 0% for "lost"
        oppToUpdate.chance_of_winning = 0;
      }
    }
    ...
    axios
      .put(
        `http://localhost:8081/opportunities/${id}`,
        {
          ...
          // Include the status field in the request
          status: oppToUpdate.status,
          // Include opportunity_win_date in the request
          opportunity_win_date: oppToUpdate.opportunity_win_date,
          // Include chance_of_winning in the request
  ...
  const handleViewClick = (id) => {
    localStorage.setItem("current_opp_id", id);
                ...
                <tr key={opp.id}>
                  ...
                  <td className="text-center">
                    <button
                      ...
                      onClick={() => handleViewClick(opp.id)}
                    >
                      View
                    </button>
                    <button
